...
namespace GalacticScale
{
    public partial class PatchOnPlanetSimulator
    {
        ...
        {
	        // Defensive guard (0.10.35): vanilla's own LateRefresh() dereferences planetData.star
	        // directly, and PlanetData.ambientDesc is a class that the game explicitly nulls out
	        // during teardown (PlanetData.cs: obj.ambientDesc = null). This replacement copy has the
	        // same unguarded access, so whenever a simulator ticks before its PlanetData is fully
	        // wired (or after the planet was released) it threw one NullReferenceException per frame
	        // -- ~10.7k of them in a single session on 0.10.35, drowning the log and taking
	        // GpuAnalysis down with it.
	        //
	        // Returning true instead of false here abandons the replacement for this call and lets
	        // the original PlanetSimulator.LateUpdate() (LateRefresh + GpuAnalysis) run. That keeps
	        // sky/atmosphere rendering alive in states GS2 was never able to render anyway, rather
	        // than silently skipping the frame. Note this branch cannot double-run GpuAnalysis:
	        // our own call to it sits at the very end of the block below, after this guard.
	        if (__instance == null || __instance.planetData == null || __instance.planetData.star == null)
	        {
		        return true;
	        }
			...
			// ambientDesc is a MonoBehaviour-derived class that the game nulls out during teardown
			// (PlanetData.cs: obj.ambientDesc = null), so this block needs its own guard on top of
			// the planetData/star one above.
			if (__instance.oceanMat != null && planetData.ambientDesc != null)
			{
